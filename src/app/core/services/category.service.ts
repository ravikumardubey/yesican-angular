...
import { CATEGORY_DETAIL, CATEGORY_LIST } from '../constants/api.constants';
...
export class CategoryService {
  ...
  createCategory(data: any): Observable<any> {
    return this.http.post(`${this.baseUserUrl}${CATEGORY_LIST}`, { ...data });
  }
  getCategories(searchQuery = ''): Observable<any> {
    ...
    return this.http.get<any>(`${this.baseUserUrl}${CATEGORY_LIST}`, {
      params,
    });
  }
  ...
  getAllCategories(page = '1'): void {
    ...
    this.http
      .get<any>(`${this.baseUserUrl}${CATEGORY_LIST}`, { params })
      .subscribe({
        next: (res) => {
          if (res) {
  ...
  getCategoriesForHome(): void {
    this.http.get<any>(`${this.baseUserUrl}${CATEGORY_LIST}`).subscribe({
      next: (res) => {
        if (res) {
          const data = new PaginatedData({
            ...res,
  ...
  updateCategory(id: string, data: any): Observable<any> {
    return this.http.put<any>(
      `${this.baseUserUrl}${CATEGORY_DETAIL}`.replace(`:id`, `${id}`),
      { ...data }
    );
  }
